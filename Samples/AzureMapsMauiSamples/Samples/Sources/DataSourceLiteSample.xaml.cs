...
public partial class DataSourceLiteSample : ContentPage
{
    /*********************************************************************************************************
    * The DataSourceLite class is a simplified version of the DataSource class which only maintains a copy of 
    * the data within the map in JavaScript. Since there isn't a .NET copy of the data, this data source uses 
    * half the memory, but requires making async requests to get insights on the data source, like the number 
    * of features it contains.
    * 
    * This sample is nearly identical to the Data Source Import Sample. 
    * 
    * Also see the DrawingToolsDataAccessSample as the drawing manager uses the DataSourceLite class.
    * 
    ...
    *********************************************************************************************************/
    ...
    private void MyMap_OnReady(object sender, MapEventArgs e)
    {
        ...
        //Create a lite data source and add it to the map.
        ...
        //Add a click events to the layers.
        MyMap.Events.Add("click", [polygonLayer, lineLayer, pointLayer], Layer_Click);
    ...
    private void ImportFeatureShape_Clicked(object sender, EventArgs e)
    {
        ...
        */
        ...
        dataSource.Add(shape); //Using async add here to wait for this to complete before updating camera. 
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private async void ImportFromStream_Clicked(object sender, EventArgs e)
    {
        //Close the popup.
        popup.Close();
        ...
        //Get the GeoJson file from the app package.
        using (var stream = await FileSystem.Current.OpenAppPackageFileAsync("map_resources/data/geojson/SamplePoiDataSet.json"))
        {
            //Importing asynchroneously so that when we update the camera, it will be after the data is populated.
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private async void ImportRawFolderFileUrl_Clicked(object sender, EventArgs e)
    {
        //Close the popup.
        popup.Close();
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private async void ImportWebUrl_Clicked(object sender, EventArgs e)
    {
        //Close the popup.
        popup.Close();
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private async void ImportWithFilePicker_Clicked(object sender, EventArgs e)
    {
        //Close the popup.
        popup.Close();
        ...
        //Make sure file exists and that it's extension is .json or .geojson.
        if (file != null)
        {
            if (file.FileName.EndsWith(".json", StringComparison.OrdinalIgnoreCase) ||
                file.FileName.EndsWith(".geojson", StringComparison.OrdinalIgnoreCase) ||
                file.FileName.EndsWith(".js", StringComparison.OrdinalIgnoreCase))
            {
            ...
            {
                await DisplayAlert("Alert", "File type not supported.", "OK");
            }
        }
    }
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private async void ImportAzureMapsRestGeoJsonResponse_Clicked(object sender, EventArgs e)
    {
        /************************
         * NOTE: 
         * 
        ...
        //Behind the scenes this will notice that the request is to an Azure Maps REST service and will sign the request with the same Azure Maps auth info used by the map.
        await dataSource.ImportDataFromUrlAsync(geocodeRequest);

        UpdateCamera();
    }

    private async void ImportGenericRestResponse_Clicked(object sender, EventArgs e)
    {
        //Close the popup.
        popup.Close();
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void ClearDataSource_Clicked(object sender, EventArgs e)
    {
        //Close the popup.
        popup.Close();
    ...
    private async void UpdateCamera()
    {
        ...
        var bounds = await dataSource.GetBoundsAsync();
